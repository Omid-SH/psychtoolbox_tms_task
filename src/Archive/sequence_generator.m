% This code will make a 96 trials sequence for Day1 
% or                    24 trials sequence for Day2 


% *************************** README **********************************
% Sequence trial types map [ S(1,:) ]
% force -> 1
% choice1 -> 2
% choice2 -> 3
% choice2' -> 4 (two fraktals in one side)
% passive viewing -> 5
% choice1' -> 6
% rest1 -> 7
% probe -> 8

% Sequence trial method map [ S(2,:) ]
% 1) force -> ab ==> a: fracktal number, b: 1-> left 2-> right

% 2) choice1 -> ab ==> a: fracktal number, b: 1-> left 2-> right

% 3) choice2 -> ab ==> a: left fracktal number, b: rigth fracktal number

% 4) choice2' -> abc ==> a: up fracktal number, b: down fracktal number,
%                        c: 1-> left, 2-> right

% 5) passive viewing -> abc ==> a: fracktal number, b: 1-> left 2-> right,
%                              c: 1-> top, 2-> center, 3-> bottom

% 6) choice1' (with fake fraktals and maybe opposite side show) -> abc 
% ==> a: fracktal number ( may be more than 10!) , b: 1-> left 2-> right, c: 1-> good, 2-> bad, 3->novel
...
l_index = input('Subject l index? ');
h_index = input('Subject h index? ');

data_index = input('Subject dataset index? ');
Day = input('Sequence for day 1, 2, or 3 (or 4 -> 24 choice1, 5 -> 96 Day2 with one hemifield choice2, 6 -> 128 Day2, 7 -> 128 Day2 with one hemifield choice2)?, 8 -> 72(24 on same side of training + 24 opposite side show + 24 fake),  9 -> 48(12 probe 2 + 12 probe 3 + 24 rest)? ');
